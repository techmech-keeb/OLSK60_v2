...
>
> **テスト版のファームウェアは 5-Split Space と 3-Split Space の 2 種類**です。トッププレートに合ったファイルを書き込みます（6.25U Space 用は用意していません）。
...
- **お使いのスペース列に合うファイル**（`5split` / `3split`）を書き込みます。取り違えるとキー配置がずれます。
