...
- 6.25U Space … 6.25U 用スタビライザーが別途必要です（キットには含まれません）。**テスト版には 6.25U 用のプリセットがありません**ので、いずれかを書き込んだうえで Vial で割り当ててください
...
4. テスト版を書き込む場合は、**お使いのスペース列に合うプリセット**（`5split` / `3split`）を選ぶと、そのまま調整なしで使えます（機能はどちらも同じです）。詳しくは [firmware/README.md](../firmware/README.md) をご参照ください。
