# OLSK60 v2 ビルドガイド

このドキュメントは、OLSK60 v2（格子配列60%キーボード）の組み立て手順をまとめたものです。  
キットの内容物や別途必要なパーツについては、[BOOTHの商品ページ](https://techmech.booth.pm/items/5896343)または[遊舎工房の商品ページ](https://shop.yushakobo.jp/products/11324)をご確認ください。

> **初心者の方へ:** セクション1「カスタマイズのための準備」はスタビライザー交換やフォーム貼り付けなど、カスタマイズ目的の作業です。  
> まずは組み立てたい場合は、[セクション2「キースイッチの取り付け」](#2-キースイッチの取り付け)から始めてください。

> **モデルについて:** 現在販売中のモデルは **v2.1** で、出荷時は **5-Split Space** レイアウトです。同梱のトッププレートで 3-Split Space／6.25U Space にも変更できます。旧モデル **v2** は 3-Split Space および 6.25U Space のみに対応しています。

---

## 1. カスタマイズのための準備（任意）

スイッチプレートの交換、フォーム貼り付け、スタビライザー交換などのカスタマイズを行う場合に、このセクションの手順を実施してください。カスタマイズが不要な場合はスキップできます。

### 1.1 基本セットの分解手順

スイッチプレートの交換やフォーム貼り付け、スタビライザー交換を行う場合は、以下の手順で基本セットを分解してください。

<!-- 写真: images/build/01-flat-cable-disconnect.jpg（フラットケーブルを外す手元のアップ） -->
<!-- 写真: images/build/02-disassembled-parts.jpg（分解後の部品を並べた俯瞰） -->

1. 基板とスイッチプレートを分解する前に、トラックポイントとメイン基板を接続するフラットケーブルをコネクターから外します。

   > ⚠️ **注意:** フラットケーブルのコネクターは非常にもろいため、無理な力を加えず慎重に取り扱ってください。

2. 基板とスイッチプレートがネジで固定されている場合は、付属のドライバーで全てのネジを外します。
3. スイッチプレートと基板をゆっくりと分離します。トラックポイントユニットやケーブルが引っかからないよう注意してください。
4. 必要に応じて、スタビライザーやフォームを取り外します。
5. 必要なパーツの交換・貼り付け・取り付け作業を行ってください。
6. 作業が完了したら、基板とスイッチプレートを元通りに重ね、ネジでしっかりと固定します。

> ⚠️ **注意:** 組み立て済みのトラックポイントユニットを無理に外そうとすると破損の原因となります。交換や調整が必要な場合は十分ご注意ください。

### 1.2 トッププレート（スイッチプレート）の交換（レイアウト変更）

OLSK60 v2.1 は、出荷時に **5-Split Space（1U・1U・1.25U・1U・1.75U）** レイアウトのトッププレートが取り付けられています。最下段のスペースバー構成を変更したい場合は、同梱の **3-Split Space／6.25U Space 兼用トッププレート** に付け替えてください。

> **補足:** 旧モデル v2 は 3-Split Space（2.25U・1.25U・2.75U）および 6.25U Space のみに対応しており、5-Split Space 用トッププレートは使用できません。

<!-- 写真: images/build/03-top-plates.jpg（2枚のトッププレートの比較） -->

1. [セクション1.1](#11-基本セットの分解手順)の手順で基本セットを分解します。
2. トッププレートを使用したいレイアウトのものに交換します。
3. レイアウトに合わせてスタビライザーの位置を調整します（[セクション1.3](#13-スタビライザーの交換)参照）。
4. 交換後は、設定ツール（Remap / VIA / Vial）でレイアウトオプションを切り替え、**Space などのキーを割り当て直してください**。出荷時キーマップは 5-Split Space 用のため、3-Split／6.25U ではそのままでは Space がありません。詳しくは[操作ガイドのレイアウトオプション](OLSK60_user_guide.md#スペースバーのレイアウトオプション)をご参照ください。

### 1.3 スタビライザーの交換

スタビライザーは、選択するスペースバーレイアウトに応じて取り付け位置が変わります。

**出荷時（v2.1・5-Split Space）に取り付け済みの位置:**
- バックスペースキー（2U）
- エンターキー（2U）

5-Split Space レイアウトでは、最下段の各キーが小さいためスペースバー用のスタビライザーは使用しません。同梱の予備スタビライザー（2U×2）は、他のレイアウトへ変更する際にご使用ください。

**他のレイアウトに変更する場合:**
- 3-Split Space … 2.25U と 2.75U の位置に 2U スタビライザーを追加（同梱の予備を使用）
- 6.25U Space … 6.25U 用スタビライザーが別途必要です（キットには含まれません）

![スタビライザーの取り付け位置図。5-Split はバックスペースとエンターの2箇所、3-Split では最下段に2箇所追加](images/stab-positions.svg)

> **補足:** 旧モデル v2 は、出荷時にバックスペース・エンターに加えて 2.25U・2.75U スペースバー用のスタビライザーも取り付けられています。6.25U Space に変更する場合は 6.25U 用スタビライザーを別途ご用意ください。

<!-- 写真: images/build/04-stab-positions.jpg（スタビライザー取り付け済み基板の俯瞰） -->

1. 必要な位置にスタビライザーを交換・取り付けます。
2. しっかりと固定されていることを確認してください。

### 1.4 スイッチフォームの設置

<!-- 写真: images/build/05-switch-foam.jpg（フォームをPCBに重ねた状態の俯瞰） -->

1. スイッチフォームをPCB上に配置します。
2. 穴位置がずれていないか確認してください。

### 1.5 トラックポイントユニットの固定（必要に応じて）

1. トラックポイントユニットがPCBにしっかりと固定されているか確認します（キット付属・組立済みです）。
2. ユニットが緩んでいる場合は、付属のネジやナットでしっかりと固定してください。
3. 必要に応じて動作確認を行ってください。

### 1.6 メイン基板とスイッチプレートの組み立て

<!-- 写真: images/build/06-flat-cable-connect.jpg（フラットケーブル再接続のアップ。向きとロック状態） -->

1. スイッチフォームやスタビライザー、トラックポイントユニットの取り付けが完了したら、メイン基板とスイッチプレートを重ねます。
2. 付属のネジでしっかりと固定します。
3. トラックポイントとメイン基板を接続するフラットケーブルをコネクターに再度接続します。

   > ⚠️ **注意:** コネクターは非常にもろいため、無理な力を加えず慎重に取り扱ってください。

---

## 2. キースイッチの取り付け

<!-- 写真: images/build/07-switch-pins.jpg（スイッチのピンのアップ） -->
<!-- 写真: images/build/08-socket-support.jpg（ソケットを下から押さえる手元） -->

1. 取り付ける前に、各キースイッチのピンが曲がっていないかよく確認してください。
2. スイッチをスイッチプレートとPCBにまっすぐゆっくりと差し込みます。位置を確かめながら、無理な力を加えずに差し込んでください。

   > **ヒント:** 差し込み時は下側からソケットを軽く抑えながら作業すると、ソケットの破損防止になります。

3. すべてのキースイッチを差し込んだら、[Remap](https://remap-keys.app/)に接続し、マトリクステスターで全てのキーが正しく動作するか一つずつ確認してください。RemapはWebブラウザ上で動作するキーボード設定ツールです。

---

## 3. キーキャップの取り付け

1. 各キースイッチにキーキャップを取り付けます。
2. 配列・サイズに注意してください。出荷時の 5-Split Space レイアウトでは、最下段に 1U キー 3 個と 1.75U キー 1 個を使用します。

---

## 4. ケースへの組み込み

1. 組み立てたPCB＋プレート＋スイッチをGH60互換ケースに組み込みます。
2. ケース付属のネジで固定します。

---

## 5. ファームウェアの確認

OLSK60 v2は出荷時にファームウェアが書き込まれています。通常は書き込み作業は不要です。出荷時のファームウェアは 5-Split Space レイアウトに対応しています。

> **補足:** 2026年6月までにご購入の個体には旧版ファームウェアが入っています。そのままご利用になる場合は [Remap](https://remap-keys.app/) でご利用いただけます（更新不要）。

ファームウェアの更新が必要な場合は、[firmware/README.md](../firmware/README.md)をご参照ください。

1. USBケーブルでPCと接続します。
2. すべてのキー・トラックポイントが正常に動作するか確認してください。
3. キーマップのカスタマイズには [Remap](https://remap-keys.app/) を使用できます。RemapはVIA互換のWebアプリで、ソフトウェアのインストールなしにキーマップを編集できます。
4. 最新版に更新する場合は、VIA 用または Vial 用のファームウェアを書き込み、VIA（付属の JSON を読み込み）または Vial（**Pipette 推奨**）をご利用ください。詳しくは [firmware/README.md](../firmware/README.md) をご参照ください。

---

## 6. トラブルシューティング

| 症状 | 対処方法 |
|------|----------|
| キーが反応しない | スイッチを抜き差しし、ピンが曲がっていないか確認してください。改善しない場合はソケットの状態を確認してください |
| 複数キーが反応しない | フラットケーブルの接続を確認してください（[セクション1.1](#11-基本セットの分解手順)参照） |
| トラックポイントが動作しない | フラットケーブルの接続を確認してください。ケーブルを抜き差しして再起動もお試しください |
| キーボードがPCに認識されない | USBケーブルを抜き差し → 別のUSBポートを試す → 別のケーブルを試す |
| スタビライザーが引っかかる | スタビライザーが正しい位置にはまっているか、ワイヤーが外れていないか確認してください |

---

## 7. 次のステップ

組み立てが完了したら、以下のドキュメントで使い方をご確認ください。

- [キーボード操作ガイド](OLSK60_user_guide.md) — レイヤー構成や各機能の使い方
- [マウス機能操作マニュアル](OLSK60_mouse_manual.md) — トラックポイントの速度調整

---

## 8. サポート

ご不明点があれば[BOOTHの商品ページ](https://techmech.booth.pm/items/5896343)または[GitHub Issue](https://github.com/techmech-keeb/OLSK60_v2/issues)にてご連絡ください。

---

以上で組み立ては完了です。お疲れ様でした！
