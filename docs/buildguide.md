...
![組み立ての構成順を示す分解図。上からキーキャップ、キースイッチ、トッププレート、スイッチフォーム（別売）、メイン基板、ケースの順。トラックポイントユニットはトッププレートの上でキースイッチの間に設置され、FFCはトッププレートとPCBの穴を通ってPCB裏面のコネクターへ接続する](images/build-exploded.svg)

1. 基板とスイッチプレートを分解する前に、トラックポイントとメイン基板を接続するフラットケーブル（FFC）を、**PCB裏面のコネクター**から外します。FFCはトッププレートとPCBの穴を通って裏面に回り込んでいます。

   ![フラットケーブルの取り扱い注意図。端子部を持ちまっすぐゆっくり抜き差しする。斜めに引っ張る・ねじる・強い力を加える・鋭角に折り曲げるのは禁止](images/flat-cable-handling.svg)
...
4. 交換後は、設定ツール（Remap / VIA / Vial）でレイアウトオプションを切り替え、**Space などのキーを割り当て直してください**。出荷時キーマップは 5-Split Space 用のため、3-Split／6.25U ではそのままでは Space がありません。詳しくは[操作ガイドのレイアウトオプション](OLSK60_user_guide.md#スペースバーのレイアウトオプション)をご参照ください。
...
![スタビライザーの取り付け位置図。5-Split はバックスペースとエンターの2箇所、3-Split では最下段に2箇所追加](images/stab-positions.svg)
...
1. **厚さ 3.5mm** のフォームをご用意ください。トッププレート下面とPCB上面の間隔が 3.5mm のため、この厚みがちょうど収まります（[2.1 スイッチの寸法](#21-スイッチの寸法)参照）。
2. 穴位置を合わせてPCB上に配置します（重ねる順序は[セクション1.1の分解図](#11-基本セットの分解手順)参照）。スイッチのピン・スタビライザー・トラックポイントユニットの取り付け部と干渉しないか確認してください。
...
1. スタビライザー（フォームを使う場合はフォームも）とトラックポイントユニットの取り付けが完了したら、メイン基板とスイッチプレートを重ねます。
2. 付属のネジでしっかりと固定します。
...
1. 取り付ける前に、各キースイッチのピンが曲がっていないかよく確認してください。
   ...
   ![取り付け前のピン確認図。ピンがまっすぐなら良い。曲がっている場合はまっすぐに直してから取り付ける](images/switch-pins.svg)

2. スイッチをスイッチプレートとPCBにまっすぐゆっくりと差し込みます。位置を確かめながら、無理な力を加えずに差し込んでください。

   ![キースイッチの差し込み方の断面図。真上からまっすぐ差し込み、下から指でソケットを軽く支える](images/switch-mount.svg)
...
OLSK60 は **MX互換**のキースイッチに対応しています。スイッチを別途購入する場合や、プレート・キーキャップとの寸法関係を確認したい場合は次の図を参照してください。

![MX互換キースイッチの三面図。正面図・側面図・上面図・底面のピン配置・プレートマウント断面を、寸法（mm）付きで示す](images/cherry-mx-switch-views.svg)
...
|---|---|
| 全高（ステム頂点〜ピン先端） | 18.5 |
| トップハウジング外形 | 15.6 × 15.6 |
| プレート開口（下部ハウジングが通る部分） | 14.0 × 14.0 |
| プレート厚 | 1.5 |
| キースイッチピッチ（1U） | 19.05 |

> 各部の内訳・PCB フットプリントの座標・ステムの参考値は、[MX互換スイッチ 寸法リファレンス](reference/cherry-mx-switch-spec.md)にまとめています。

OLSK60 側の積層寸法は次のとおりです（[差し込み方の断面図](#2-キースイッチの取り付け)と対応）。
...
| トッププレート下面〜PCB上面 | 3.5 |
| PCB厚 | 1.6 |
| ホットスワップソケット厚（PCB裏面） | 2.0 |

スイッチのハウジングはプレート上面から 4.8mm 下まで、ピン先端は 8.1mm 下まで伸びるため、ピンは PCB を貫通して裏面のソケットに届きます。

> トッププレート下面とPCB上面の 3.5mm の空間は、**別売のスイッチフォーム（3.5mm厚）**がちょうど収まる厚みです。フォームはキットに含まれないため、使用する場合はご自身でご用意ください（[1.4 スイッチフォームの設置（任意）](#14-スイッチフォームの設置任意)参照）。
...
![5-Split Space レイアウトの全キーキャップサイズ図。各キーに刻印名と幅（U）を表示し、サイズ別に色分け。下部にサイズ別の必要数を掲載](images/keycap-sizes-5split.svg)

**3-Split Space（v2 出荷時 / v2.1 でも選択可）**

![3-Split Space レイアウトの全キーキャップサイズ図。各キーに刻印名と幅（U）を表示し、サイズ別に色分け。下部にサイズ別の必要数を掲載](images/keycap-sizes-3split.svg)

**6.25U Space（v2 / v2.1）**

![6.25U Space レイアウトの全キーキャップサイズ図。各キーに刻印名と幅（U）を表示し、サイズ別に色分け。下部にサイズ別の必要数を掲載](images/keycap-sizes-625u.svg)
