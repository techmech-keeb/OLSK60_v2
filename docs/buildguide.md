...
4. 交換後は、**そのスペース列に対応したファームウェアを書き込んでください**。テスト版ではスペース列ごとにファイルが分かれており（`5split` / `3split`）、対応するファイルを書き込めば Space などの割り当ては最初から正しく載ります（6.25U 用は用意していません）。詳しくは[操作ガイドのレイアウトオプション](OLSK60_user_guide.md#スペースバーのレイアウトオプション)をご参照ください。
...
- 6.25U Space … 6.25U 用スタビライザーが別途必要です（キットには含まれません）。**テスト版のファームウェアには 6.25U 用のファイルがありません**（5-Split / 3-Split のみ）
...
> **補足:** テスト版のファームウェアは Vial 専用です。更新しない場合は、お手元のファームウェアに対応した設定ツールをそのままお使いください。

ファームウェアの更新が必要な場合は、[firmware/README.md](../firmware/README.md)をご参照ください。

1. USBケーブルでPCと接続します。
2. すべてのキー・トラックポイントが正常に動作するか確認してください。
3. キーマップのカスタマイズには [Vial](https://get.vial.today/) を使用します。キーボード定義はファームウェアに内蔵されているので、定義ファイルの読み込みは不要です。
4. テスト版を書き込む場合は、**お使いのスペース列に合うファイル**（`5split` / `3split`）を選んでください。詳しくは [firmware/README.md](../firmware/README.md) をご参照ください。
