...
3. **トラックポイント機能**
   - オートマウスレイヤー機能
トラックポイントを操作している間は自動的にキーマップのレイヤーが切り替わり、最下段のスペース/レイヤーキー/デリートキーがマウスキー（左ボタン/スクロールボタン/右ボタン）となります。

   - トラックポイントの感度調整が可能です。
   - トラックポイントDepress機能

この機能により、トラックポイントを軽く押し下げることでクリック操作が可能になります。今後のアップデートでRemapから簡単にON/OFFを切り替え可能にする予定です。なお、現時点では操作時に誤動作は発生しないため、基本的にはONで起動する仕様になっています。
